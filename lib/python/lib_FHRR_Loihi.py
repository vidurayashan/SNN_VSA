...
#  Search for this variable to convert real to complex calculations
enable_complex_cal = True 
...
def set_params_auto_custom_max(error_percentage, R, max_val):
    GlobalVars.global_threshold = round((max_val)*(1 - error_percentage) / error_percentage)
    GlobalVars.time_steps       = np.ceil( GlobalVars.global_threshold / R )
    # print(f'GlobalVars.global_threshold: {GlobalVars.global_threshold}')
    # print(f'GlobalVars.time_steps      : {GlobalVars.time_steps}')
...
class FHRR_Encoder_Latency(AbstractProcess):
    def __init__(self, vec, sim_time=1, phase=1):
        super().__init__()
        hd_vec = vec
        shape = (len(hd_vec),)
        self.spikes_out = OutPort(shape=shape)  # Input spikes to the classifier
        ...
        self.phase    = Var(shape=(1,), init=phase)
...
class FHRR_Decoder(AbstractProcess):
    ...
    def reset_decoded_value(self, dimension):
        self.v.set(np.zeros(dimension))

    def get_spike_period(self):
        return self.a_period.get()
...
class PySpikeInputModelLatency(PyLoihiProcessModel):
    ...
    phase: int = LavaPyType(int, int, precision=32)
...
#         print(f"decoded_b         : {[item * 180 / cmath.pi for item in decoded_b]}")
        ...
        #####  enable_complex_cal end ####

        sum_real = [a + b for a,b in zip(decoded_a, decoded_b)]
        self.phase_arr_clean = sum_real
...
#         print(f"decoded_b         : {[item * 180 / cmath.pi for item in decoded_b]}")
        ...
        mult_phase_cleaned = [a * b for a,b in zip(decoded_a, decoded_b)]
